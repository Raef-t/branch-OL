...
export default function Step6EnrollmentContract({
  ...
  useEffect(() => {
    // Default initial payment institute branch from Step 1 (passed as prop)
    const bid = instituteBranchId != null ? String(instituteBranchId) : "";
    if (bid !== "" && !form.first_payment.institute_branch_id) {
      handleFirstPaymentChange("institute_branch_id", bid);
    }
  }, [instituteBranchId]);
  ...
  const computed = useMemo(() => {
    ...
    // ✅ مهم: الحسم النهائي الذي يذهب للباك بالدولار دائمًا
    ...
    return {
      rate,
      baseAmountLocal,
      discountAmountLocal, // للعرض فقط
      discountAmountUsd, // للإرسال للباك
      discountPercentage,
      finalAmountLocal,
      totalUsd,
      finalUsd,
  ...
  useEffect(() => {
    ...
    setInstallments((prev) => {
      const next = Array.from({ length: count }, (_, idx) => {
        ...
        // إذا كان القسط موجوداً سابقاً وله قيمة، نترك قيمته (اختياري، بس الزبون بدو توزيع)
        // الأفضل نوزع إذا الحقل كان فاضي أو إذا تغير العدد
        return {
  ...
  const buildPayloadBase = () => {
    ...
    // ✅ discount_amount يُرسل بالدولار دائمًا
        ...
        <div className="space-y-4">
          ...
          <InputField
            ...
            onBlur={() => {
              ...
              if (!Number.isFinite(n)) return;
              handleDiscountPercentageChange(String(clampNumber(n, 0, 100)));
            }}
            placeholder="مثال: 10"
          />
            ...
            {currency === "usd" ? (
              <>
                ...
                <div>
                  قيمة الحسم:{" "}
                  <span className="font-semibold">
                    {Number(computed.discountAmountLocal || 0).toFixed(2)} USD
                  </span>
                </div>
                ...
                {form.first_payment.currency === "USD" ? (
                  ...
                  <>
                    <InputField
                      ...
                      onChange={(e) => {
                        const v = onlyNumberString(e.target.value);
                        handleFirstPaymentChange("exchange_rate_at_payment", v);
                      }}
                    />
                    <InputField
