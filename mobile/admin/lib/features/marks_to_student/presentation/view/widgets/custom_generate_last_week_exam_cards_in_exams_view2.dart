...
import '/features/marks_to_student/presentation/view/widgets/custom_success_state_last_marks_in_marks_view.dart';

class CustomGenerateLastWeekExamCardsInExamsView2 extends StatelessWidget {
  const CustomGenerateLastWeekExamCardsInExamsView2({super.key});
    ...
    return BlocBuilder<MarkStudentsCubit, MarkStudentsState>(
      builder: (context, state) {
        if (state is MarkStudentsSuccessState) {
          final lastWeekList = state.markTwoWeeksModelInCubit.lastWeekList;
          final length = lastWeekList.length;
          if (lastWeekList.isEmpty) {
            return const TextSuccessStateButTheDataIsEmptyComponent(
              text: 'لا يوجد علامات',
            );
          }
          return CustomSuccessStateLastMarksInMarksView(
            length: length,
