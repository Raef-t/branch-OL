...
class CustomWorkHoursToBatchViewBody extends StatefulWidget {
  const CustomWorkHoursToBatchViewBody({super.key});

  @override
  State<CustomWorkHoursToBatchViewBody> createState() =>
      _CustomWorkHoursToBatchViewBodyState();
}

class _CustomWorkHoursToBatchViewBodyState
    extends State<CustomWorkHoursToBatchViewBody> {
      ...
      slivers: [
        const SliverAppBarToHoleAppComponent(
          appBarWidget: AppBarWidgetWithRightArrowImageAndTwoTextsComponent(
            ...
            secondText: 'يمكنك الاطلاع على برنامج دوام الشعبه',
          ),
        ),
        SliverFillRemaining(
          hasScrollBody: false,
              ...
              children: [
                TextFullDateAndFullDateCardComponent(
                  ...
                  onDateSelected: (selectedDateInFunction) {
                    ...
                    context.read<ScheduleToBatchCubit>().getSchedule(
                      day: formattedDate,
                    );
                  },
                ),
                ...
                CustomGenerateWorkHoursToBatch(
...
class CustomGenerateWorkHoursToBatch extends StatelessWidget {
  const CustomGenerateWorkHoursToBatch({
    super.key,
  ...
  @override
  Widget build(BuildContext context) {
    return BlocBuilder<ScheduleToBatchCubit, ScheduleToBatchState>(
      builder: (context, state) {
        if (state is ScheduleToBatchSuccessState) {
        ...
        } else if (state is ScheduleToBatchFailureState) {
